"""Gradio dashboard for the AI Cyber-Defense Pipeline v2.0 — Aikido-style UI."""
...
<div style="height:100vh;overflow-y:auto;color:#a0aec0;font-size:14px;">
    ...
    <div style="padding:18px 20px;display:flex;align-items:center;gap:10px;">
        ...
        <span style="color:white;font-size:16px;font-weight:700;letter-spacing:-0.3px;">CyberDefense</span>
    </div>
...
def _format_summary_cards(result: dict | None = None) -> str:
    """Build the four Aikido-style summary cards."""
    if result is None:
        return """
...
def _format_filter_bar() -> str:
    """Build the Aikido-style search / filter toolbar."""
    return """
...
def _format_threats_table(classified_threats: list[ClassifiedThreat] | None = None) -> str:
    """Build the Aikido-style findings data table."""
...
with gr.Blocks(title="CyberDefense \u2014 Security Dashboard") as demo:
