...
Comprehensive reference for the AI Cyber-Defense Pipeline internals — data models, REST API, agents, rule engine, pipeline orchestration, CLI, and Next.js dashboard.
...
Global state via React Context (`AnalysisProvider`) wrapping the entire app in `layout.tsx`. State persists to `localStorage` under the key `cyberdefense_analysis`.
